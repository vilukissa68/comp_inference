...
PYBIND11_MODULE(ccore, m) {
    ...
    m.def(
        ...
        [](
            // Inputs
            at::Tensor stream, at::Tensor states, at::Tensor sizes,
            uint32_t num_streams, at::Tensor tables, at::Tensor slot_map,
            at::Tensor output) {
            TORCH_CHECK(stream.is_cuda(), "Stream must be on GPU");
            TORCH_CHECK(output.is_cuda(), "Output buffer must be on GPU");

            decompress(
                (uint8_t *)stream.data_ptr<uint8_t>(), stream.numel(),
                reinterpret_cast<const uint32_t *>(states.data_ptr<int32_t>()),
                reinterpret_cast<const uint32_t *>(sizes.data_ptr<int32_t>()),
                num_streams, output.numel(), tables.data_ptr<uint32_t>(),
    ...
    m.def(
        ...
        [](size_t size) {
            ...
            py::capsule free_when_done(ptr, [](void *p) { cudaFreeHost(p); });

            return py::array_t<uint8_t>({size}, {sizeof(uint8_t)},
                                        (uint8_t *)ptr, free_when_done);
        },
        "Allocates a pinned numpy array");
    m.def(
        "allocate_pinned_tensor",
        [](size_t size) {
            void *ptr = nullptr;
            ...
            return torch::from_blob(ptr, {static_cast<long>(size)}, deleter,
                                    options);
        },
    ...
    py::class_<TensorCompressResult>(m, "TensorCompressResult")
        ...
        .def_readonly("tables", &TensorCompressResult::tables)
    ...
    py::class_<RansManager::CompressResult>(m, "CompressResult")
        ...
        .def_property_readonly("stream",
                               [](const RansManager::CompressResult &r) {
                                   return py::array_t<uint8_t>(r.stream.size(),
                                                               r.stream.data());
                               })
        .def_property_readonly("states",
                               [](const RansManager::CompressResult &r) {
                                   return py::array_t<uint32_t>(
                                       r.states.size(), r.states.data());
                               })
        .def_property_readonly("tables",
                               [](const RansManager::CompressResult &r) {
                                   return py::array_t<uint32_t>(
                                       r.tables.size(), r.tables.data());
                               })
        .def_property_readonly("slot_map",
                               [](const RansManager::CompressResult &r) {
                                   return py::array_t<uint8_t>(
                                       r.slot_map.size(), r.slot_map.data());
                               })
        .def_property_readonly("output_sizes",
                               [](const RansManager::CompressResult &r) {
                                   return py::array_t<uint32_t>(r.sizes.size(),
                                                                r.sizes.data());
                               })
    ...
    py::class_<RansManager>(m, "RansManager")
        ...
        .def("compress",
             [](RansManager &self, py::array_t<uint8_t> data,
        ...
        .def("compress",
             [](RansManager &self, at::Tensor data, at::Tensor freqs,
                at::Tensor cdf) -> TensorCompressResult {
                 ...
                 auto stream_t =
                     torch::from_blob(res.stream.data(),
                 ...
                 auto states_t =
                     torch::from_blob(res.states.data(),
                                      {static_cast<long>(res.states.size())},
                                      opts_i32)
                         .clone();

                 auto sizes_t =
                     torch::from_blob(res.sizes.data(),
                 ...
                 auto tables_t =
                     torch::from_blob(
                         res.tables.data(),
                         {static_cast<long>(res.tables.size())},
        ...
        .def("decompress",
             [](RansManager &self, py::array_t<uint8_t> stream,
                py::array_t<uint32_t> states, py::array_t<uint32_t> sizes,
                uint32_t num_streams, size_t output_len,
                py::array_t<uint16_t> freqs, py::array_t<uint16_t> cdf) {
                 ...
                 float time_ms = self.decompress(
                     (uint8_t *)s_inf.ptr, s_inf.size, (uint32_t *)st_inf.ptr,
                     (uint32_t *)sz_inf.ptr, num_streams, output_len,
        ...
        .def("decompress",
             [](RansManager &self, at::Tensor stream, at::Tensor states,
                at::Tensor sizes, uint32_t num_streams, size_t output_len,
                at::Tensor freqs, at::Tensor cdf) {
                 ...
                 float time_ms = self.decompress(
                     ...
                     num_streams, output_len, freqs.data_ptr<uint16_t>(),
                     cdf.data_ptr<uint16_t>(), output.data_ptr<uint8_t>());
                 return py::make_tuple(output, time_ms);
             })
        .def("decompress_into",
             [](RansManager &self, py::array_t<uint8_t> stream,
                py::array_t<uint32_t> states, py::array_t<uint32_t> sizes,
                uint32_t num_streams, py::array_t<uint16_t> freqs,
                py::array_t<uint16_t> cdf, py::array_t<uint8_t> output_buffer) {
                 auto s_inf = stream.request();
                 auto st_inf = states.request();
                 auto sz_inf = sizes.request();
                 auto f_inf = freqs.request();
                 ...
                 std::cout << "Using numpy arrays decompress_into()"
                           << std::endl;
                 float time_ms = self.decompress(
                     (uint8_t *)s_inf.ptr, s_inf.size, (uint32_t *)st_inf.ptr,
                     (uint32_t *)sz_inf.ptr, num_streams, out_inf.size,
                     (uint16_t *)f_inf.ptr, (uint16_t *)c_inf.ptr,
                     (uint8_t *)out_inf.ptr);
                 return time_ms;
             })
        .def("decompress_into", [](RansManager &self, at::Tensor stream,
                                   at::Tensor states, at::Tensor sizes,
                                   uint32_t num_streams, at::Tensor freqs,
                                   at::Tensor cdf, at::Tensor output_buffer) {
            ...
            float time_ms = self.decompress(
                ...
                num_streams, output_buffer.numel(), freqs.data_ptr<uint16_t>(),
                cdf.data_ptr<uint16_t>(), output_buffer.data_ptr<uint8_t>());
            return time_ms;
        });
}
