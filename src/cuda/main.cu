...
template <typename Config> struct RansWorkspace {
    ...
    const sym_info_t *d_sym_info = nullptr;
    const symbol_t *d_symbols = nullptr;
    io_t *d_output = nullptr;
    const state_t *d_states = nullptr;
    const uint32_t *d_sizes = nullptr;
    const symbol_t *d_slot_map = nullptr;
    const io_t *d_input = nullptr;
    ...
    void resize_dec(size_t in_bytes, size_t out_bytes, uint32_t streams) {
        if (!d_slot_map)
            CUDA_CHECK(
                cudaMalloc(&d_slot_map, Config::prob_scale * sizeof(symbol_t)));
        if (!d_input)
            CUDA_CHECK(cudaMalloc(&d_input, in_bytes));
        if (!d_init_states)
            CUDA_CHECK(cudaMalloc(&d_init_states, streams * sizeof(state_t)));
        if (!d_input_sizes)
...
template <typename Config> struct StreamConfigurator {
    static KernelConfig suggest(size_t total_data_size) {
        ...
        uint32_t suggested_streams =
            (total_data_size + target_chunk_size - 1) / target_chunk_size;
        uint32_t min_streams_for_saturation =
            prop.multiProcessorCount * 4 * best_block_size;

        if (suggested_streams < min_streams_for_saturation) {
            suggested_streams = min_streams_for_saturation;
        }
        suggested_streams =
            std::min((uint32_t)total_data_size, suggested_streams);
        if (suggested_streams == 0)
            suggested_streams = 1;
...
rans_compress_cuda(RansWorkspace<Config> &ws,
    ...
    CUDA_CHECK(cudaMemcpyAsync(ws.d_symbols, host_data.data(),
    ...
    CUDA_CHECK(cudaMemcpyAsync(result.stream.data(), ws.d_output,
                               num_streams * capacity * sizeof(io_t),
                               cudaMemcpyDeviceToHost, stream));
    CUDA_CHECK(cudaMemcpyAsync(result.final_states.data(), ws.d_states,
                               num_streams * sizeof(typename Config::state_t),
                               cudaMemcpyDeviceToHost, stream));
    CUDA_CHECK(cudaMemcpyAsync(result.output_sizes.data(), ws.d_sizes,
    ...
    // Approximate output bytes for D2H bandwidth (we don't know exact size
    // until we check output_sizes, using capacity is close enough for BW
    // estimate)
    double gb_out =
        (double)(num_streams * capacity) / (1024.0 * 1024.0 * 1024.0);

    std::cout << "[Encoder Stats]\n"
              << "  H2D Copy : " << std::fixed << std::setprecision(2)
              << t_h2d.elapsed() << " ms (" << gb_in / (t_h2d.elapsed() * 1e-3)
              << " GiB/s)\n"
              << "  Kernel   : " << t_kernel.elapsed() << " ms ("
              << gb_in / (t_kernel.elapsed() * 1e-3) << " GiB/s)\n"
              << "  D2H Copy : " << t_d2h.elapsed() << " ms ("
              << gb_out / (t_d2h.elapsed() * 1e-3) << " GiB/s)\n"
              << "  TOTAL    : " << t_total.elapsed() << " ms ("
              << gb_in / (t_total.elapsed() * 1e-3) << " GiB/s)\n"
              << "  Params   : Grid=" << grid << " Block=" << block
              << " Streams=" << num_streams << "\n";
...
template <typename Config>
PinnedVector<typename Config::symbol_t>
rans_decompress_cuda(RansWorkspace<Config> &ws,
                     ...
                     uint32_t num_streams, uint32_t symbols_per_stream,
                     const uint16_t *host_freqs, const uint16_t *host_cdf) {
    using symbol_t = typename Config::symbol_t;
    using sym_info_t = typename Config::sym_info_t;
    ...
    size_t input_bytes =
        host_stream_data.size() * sizeof(typename Config::io_t);
    size_t output_bytes =
    ...
    CUDA_CHECK(cudaMemcpyAsync(ws.d_slot_map, host_slot_map.data(),
                               ...
                               cudaMemcpyHostToDevice, stream));
    CUDA_CHECK(cudaMemcpyAsync(ws.d_input, host_stream_data.data(), input_bytes,
                               cudaMemcpyHostToDevice, stream));
    CUDA_CHECK(cudaMemcpyAsync(ws.d_init_states, host_states.data(),
                               num_streams * sizeof(typename Config::state_t),
                               cudaMemcpyHostToDevice, stream));
    CUDA_CHECK(cudaMemcpyAsync(ws.d_input_sizes, host_sizes.data(),
                               num_streams * sizeof(uint32_t),
                               cudaMemcpyHostToDevice, stream));
    t_h2d.record_stop(stream);
    ...
    // --- MEASURE D2H ---
    t_d2h.record_start(stream);
    CUDA_CHECK(cudaMemcpyAsync(result.data(), ws.d_decoded_output, output_bytes,
                               cudaMemcpyDeviceToHost, stream));
    t_d2h.record_stop(stream);
    ...
    // Throughput usually defined as "Output bytes produced per second" for
    // decoders
    std::cout << "[Decoder Stats]\n"
              ...
              << " GiB/s)\n"
              << "  Kernel   : " << t_kernel.elapsed() << " ms ("
              << gb_out / (t_kernel.elapsed() * 1e-3) << " GiB/s)\n"
              << "  D2H Copy : " << t_d2h.elapsed() << " ms ("
              << gb_out / (t_d2h.elapsed() * 1e-3) << " GiB/s)\n"
              << "  TOTAL    : " << t_total.elapsed() << " ms ("
              << gb_out / (t_total.elapsed() * 1e-3) << " GiB/s)\n"
...
int main() {
    ...
    try {
        auto config =
            StreamConfigurator<RansConfig8>::suggest(original_data.size());

        std::cout << "Auto-Tuner Selected:\n"
                  << "  Block Size: " << config.block_size << "\n"
                  << "  Streams:    " << config.num_streams << "\n"
                  << "  Chunk Size: "
                  << original_data.size() / config.num_streams << " bytes\n";
        ...
        size_t size_header_overhead =
            encoded.output_sizes.size() * sizeof(uint32_t);
        size_t table_overhead = 256 * sizeof(uint16_t) * 2;

        size_t total_compressed_size = stream_payload_bytes + state_overhead +
                                       size_header_overhead + table_overhead;
        ...
        double bits_per_symbol =
            ((double)total_compressed_size * 8.0) / TOTAL_SIZE;
        double ratio = (double)TOTAL_SIZE / total_compressed_size;

        std::cout << "\n--- Data Statistics ---\n";
        std::cout << "Original Size     : " << std::fixed
                  << std::setprecision(2) << orig_mb << " MiB\n";
        std::cout << "Compressed Payload: "
                  << (double)stream_payload_bytes / (1024 * 1024) << " MiB\n";
        std::cout << "Total Size (w/ Ovh):" << total_mb << " MiB\n";
        std::cout << "Compression Ratio : " << std::setprecision(2) << ratio
                  << ":1 ("
                  << (float)total_compressed_size / TOTAL_SIZE * 100.0f
                  << "%)\n";
        std::cout << "Bits per symbol   : " << std::setprecision(4)
                  << bits_per_symbol << " bits\n";
        std::cout << "Overhead          : "
                  << (double)(state_overhead + size_header_overhead) / 1024.0
                  << " KiB\n";

        uint32_t syms_per_stream =
            (TOTAL_SIZE + encoded.num_streams - 1) / encoded.num_streams;

        std::cout << "\n--- Starting Decompression ---\n";
        auto decoded = rans_decompress_cuda<RansConfig8>(
            workspace, encoded.stream, encoded.final_states,
            encoded.output_sizes, encoded.num_streams, syms_per_stream,
            freqs.data(), cdf.data());
