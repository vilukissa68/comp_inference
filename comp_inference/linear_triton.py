...
#             # --- OPTIMIZATION 1: LATENCY HIDING ---
#             # Issue the global memory fetch for X *before* the decode loop.
#             # The GPU will fetch this in the background while the ALUs decode rANS!
#             x_tile = tl.load(
#                 x_row_ptrs[:, None] + offs_bk[None, :] * stride_ak,
#                 mask=m_valid[:, None] & (offs_bk[None, :] < K),
#                 other=0.0,
...
#             # Use static_range if available to guarantee unrolling without spilling
#             # (If using older Triton, range(BLOCK_K) with BLOCK_K: tl.constexpr works too)
#             # for i in (
#             #     tl.static_range(BLOCK_K)
#             #     if hasattr(tl, "static_range")
#             #     else range(BLOCK_K)
#             # ):
#             for i in range(BLOCK_K):
#                 mask_k = ((k_base + i) < K) & n_valid
...
#             # --- OPTIMIZATION 2: TENSOR CORE COMPUTE ---
#             # By the time we reach this, x_tile has safely arrived from global memory
#             local_acc = tl.dot(
#                 x_tile, w_tile, local_acc, out_dtype=tl.float32, allow_tf32=False
#             )
...
#         # local_acc tracks our final output for this K-tile
#         local_acc = tl.zeros((TILE_M, TILE_N), dtype=tl.float32)
...
#             # Setup an advancing pointer for X to avoid dynamic slicing
...
#                 # ---------------------------------------------------------
#                 # OPTIMIZED COMPUTE: Direct Outer-Product Accumulation
#                 # ---------------------------------------------------------
#                 x_col = tl.load(
#                     x_col_ptrs + i * stride_ak,
#                     mask=m_valid & ((k_base + i) < K),
#                     other=0.0,
#                 ).to(tl.bfloat16)

#                 # The result broadcasts perfectly into our (TILE_M, TILE_N) accumulator
...
#         # Add the local accumulator to the global accumulator for this K-tile
#         acc += local_acc
...
#         workspace = torch.zeros((SPLIT_K, N), dtype=torch.bfloat16, device=x.device)
...
#         initial_states,
#         mantissas,
#         workspace,  # Pass workspace instead of output
#         slot_map,
#         tables,
#         M_input,
#         N,
...
#         x_flat.stride(1),
#         workspace.stride(0),
#         workspace.stride(1),  # M stride of workspace
#         workspace.stride(2),  # N stride of workspace
#         TILE_M=TILE_M,
#         TILE_N=tile_n,
#         TILE_K=tile_k,
